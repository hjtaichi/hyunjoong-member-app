...
export default function DobokShowroomScreen() {
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={[styles.page, { width: contentWidth }]}>
    ...
    <PreviewCard combo={combo} previewWidth={previewWidth} gender={gender} effectiveStyle={effectiveStyle} sleeve={sleeve} fabric={fabric} topColor={topColor} pantsColor={pantsColor} neckColor={neckColor} wristColor={wristColor} chestColor={chestColor} cloudColor={cloudColor} showChest={showChest} blackBelt={canUseCloudEmbroidery} showClouds={showClouds} styleLabels={styleLabels} sleeveLabels={sleeveLabels} embroideryLayouts={embroideryLayouts} canUseCloudEmbroidery={canUseCloudEmbroidery} />
    ...
    <View style={styles.detailSection}>
      <Pressable
        ...
        accessibilityLabel={detailsExpanded ? "상세 설정 접기" : "상세 설정 펼치기"}
      >
        <View style={styles.detailToggleTextWrap}>
          <Text style={styles.detailToggleTitle}>상세 설정</Text>
          <Text style={styles.detailToggleHint}>목깃과 소매 끝 색상을 따로 변경할 수 있습니다.</Text>
        </View>
        <Text style={styles.detailToggleArrow}>{detailsExpanded ? "⌃" : "⌄"}</Text>
      </Pressable>

      {detailsExpanded ? <View style={styles.detailPanel}>
        <View style={styles.twoColumns}>
          <OptionCard title="목깃 색상" subtitle={`${neckLinkedToTop ? "상의와 동일" : "개별 설정"} · ${neckColor.position}`} swatch={neckColor.hex} onPress={() => setSheet("neck")} />
          <OptionCard title="소매 색상" subtitle={`${wristLinkedToTop ? "상의와 동일" : "개별 설정"} · ${wristColor.position}`} swatch={wristColor.hex} onPress={() => setSheet("wrist")} />
        </View>
        <View style={styles.detailResetRow}>
          <Pressable disabled={neckLinkedToTop} onPress={resetNeckColor} style={[styles.detailResetButton, neckLinkedToTop && styles.detailResetButtonDisabled]}><Text style={[styles.detailResetText, neckLinkedToTop && styles.detailResetTextDisabled]}>목깃을 상의와 동일하게</Text></Pressable>
          <Pressable disabled={wristLinkedToTop} onPress={resetWristColor} style={[styles.detailResetButton, wristLinkedToTop && styles.detailResetButtonDisabled]}><Text style={[styles.detailResetText, wristLinkedToTop && styles.detailResetTextDisabled]}>소매를 상의와 동일하게</Text></Pressable>
        </View>
      </View> : null}
    </View>
    ...
    <ColorSheet visible={sheet === "neck"} title="목깃 색상 선택" group={fabric} selectedKey={neckColor.key} onClose={() => setSheet(null)} onSelect={chooseNeckColor} />
    <ColorSheet visible={sheet === "wrist"} title="소매 색상 선택" group={fabric} selectedKey={wristColor.key} onClose={() => setSheet(null)} onSelect={chooseWristColor} />
    <EmbroiderySheet visible={sheet === "chest"} title="용·음양·수 가슴 자수 색상 선택" colors={embroideryColorOptions} selectedHex={chestColor} onClose={() => setSheet(null)} onSelect={setChestColor} />
    <EmbroiderySheet
